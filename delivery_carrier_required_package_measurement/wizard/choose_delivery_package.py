...
class ChooseDeliveryPackage(models.TransientModel):
    ...
    dimension_uom_id = fields.Many2one(
        "uom.uom",
        default=_default_dimension_uom_id,
    )
    dimension_uom_name = fields.Char(related="dimension_uom_id.name")
    package_height_required = fields.Boolean(
        related="picking_id.carrier_id.package_height_required"
    )
    package_length_required = fields.Boolean(
        related="picking_id.carrier_id.package_length_required"
    )
    package_weight_required = fields.Boolean(
        related="picking_id.carrier_id.package_weight_required"
    )
    package_width_required = fields.Boolean(
        related="picking_id.carrier_id.package_width_required"
    )
